# Chat Barber — Bot de WhatsApp para tu barbería

Bot de WhatsApp Business (Meta Cloud API) que:

1. Responde preguntas de clientes (ubicación, precios, horario) usando IA con la información de tu negocio.
...
5. Si el barbero cancela la cita (desde Google Calendar o con el comando `cancelar <id>` por WhatsApp), avisa al cliente automáticamente y lo invita a reagendar.
...
3. En Google Calendar, comparte el calendario de la barbería con el correo de la cuenta de servicio (algo como `nombre@proyecto.iam.gserviceaccount.com`) dándole permiso de "Hacer cambios en eventos".
...
Edita también `src/config/business.js` con el nombre real, ubicación, precios y horario de tu barbería.
...
## 6. Cómo cancelar una cita como barbero
...
1. El cliente escribe cualquier pregunta ("¿dónde están?", "¿cuánto cuesta el corte?") → el bot responde usando la IA con la info de `business.js`.
2. El cliente escribe "agendar" → el bot pregunta servicio, nombre, fecha y muestra horarios disponibles según Google Calendar.
3. El cliente elige horario y confirma → se crea el evento en Calendar y se guarda la cita en SQLite (`data/barber.sqlite`).
4. 1 hora antes de la cita, el bot pregunta si sigue en pie. Si el cliente dice "no", se cancela automáticamente.
5. Si el barbero cancela la cita, el cliente recibe un aviso automático y puede reagendar escribiendo "agendar".

## Estructura del proyecto

```
src/
  config/business.js          # Información editable del negocio
  services/
  ...
  jobs/
    ...
    syncBarberCancellations.js # Detecta cancelaciones hechas por el barbero en Calendar
